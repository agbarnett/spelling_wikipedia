...
other_latex = filter(df, var %in% c('beta0','gamma1','gamma2','gamma3')) # alternative for latex
...
to_table = bind_rows(other, articles) %>%
  ...
  rename('posterior probability' = 'pval',
         'Variable' = 'var') 
# make table with rate ratios (version for latex)
to_table_latex = bind_rows(other_latex, articles) %>%
  mutate(var = case_when(
    var == 'beta0' ~ 'Intercept',
    var == 'gamma1' ~ 'Authors +5',
    var == 'gamma2' ~ 'Time', # plotted below instead
...
print(xtable(to_table_latex, digits=4), include.rownames=FALSE, hline.after=FALSE, file = "reports/10_table_citations_model.tex")
```
...
The maximum number of errors per abstract was 4, but we only plot up to 2 as there were very few abstracts with 3 or 4 errors.

#### Absolute scale

```{r absolute, include=FALSE}
# regression equations:
log.mu = rep(NA, 3)
log.mu[1] = 2.9986211 -0.0110712 * (((1+0)^2)-1) # squared and centred, zero
log.mu[2] = 2.9986211 -0.0110712 * (((1+1)^2)-1) # squared and centred, one
log.mu[3] = 2.9986211 -0.0110712 * (((1+2)^2)-1) # squared and centred, two errors
mean0 = round(exp(log.mu[1])*10)/10
mean1 = round(exp(log.mu[2])*10)/10
mean2 = round(exp(log.mu[3])*10)/10
diff1 = round((exp(log.mu[1]) - exp(log.mu[2]))*10)/10
diff2 = round((exp(log.mu[1]) - exp(log.mu[3]))*10)/10
```

To provide context to the relative reduction, we estimate the absolute reduction in citations. 
After seven years, the average number of citations for an abstract with no spelling errors is `r mean0` and for an abstract with one spelling error is `r mean1` and for an abstract with two spelling errors is `r mean2`. 
The absolute reduction in citations for abstracts with one spelling error is `r diff1` and for two spelling errors is `r diff2`. 
