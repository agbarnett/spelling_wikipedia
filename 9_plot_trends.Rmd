...
print(xtable(head(worst,10), digits=0), include.rownames=FALSE, hline.after=FALSE, file = "reports/9_top_ten.tex")
```
...
```{r types}
trend_plot_type = ggplot(trends_type, aes(x=year, y=p, ymin=lower, ymax=upper, col=factor(typeo), fill=factor(typeo)))+
  geom_point()+
  geom_line()+
...
trend_plot_type
# export
ggsave(trend_plot_type, filename='figures/9_trends_by_type.jpg', width=6, height=4.5, units='in', dpi=500)
```
...
mplot
# export
ggsave(mplot, filename='figures/9_trends_without_top.jpg', width=7.5, height=4.5, units='in', dpi=600)
```
