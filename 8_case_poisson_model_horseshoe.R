...
# using corner-point instead of subtracting mean as this option is too slow
hsCode <- nimbleCode ({
  ...
  for (i in 1:n_obs) {
    count[i] ~ dpois(mu[i])
    log(mu[i]) <- log(word_count[i]) + beta0 + beta1[country[i]] + beta2[publisher[i]] + beta3[type[i]] + gamma1*authors[i]
  }
})

# create counts of errors
counts = group_by(for_model, pmid, case, typeo, country_num, publisher_num, word_count, n_authors) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate(count = ifelse(case==0, 0, count)) # controls have no errors
 
# set up data
constants = list(n_obs = nrow(counts), 
                 word_count = counts$word_count/100, # for offset (scaled to 100 words)
                 authors = (sqrt(counts$n_authors)-2.2)/1, # transformed and scaled 
                 country = counts$country_num,
                 publisher = counts$publisher_num,
                 type = as.numeric(as.factor(counts$typeo)),
                 n_publisher = n_publisher,
                 n_country = n_country,
                 n_type = n_type)
data = list(count = counts$count) # spelling error count

# initial values
inits = list(beta0 = 0, gamma1 = 0, 
             beta1 = rep(0, n_country), beta2 = rep(0, n_publisher), beta3 = rep(0, n_type),
             tau1 = 1, lambda1 = 1, tau2 = 1, lambda2 = 1, tau3 = 1, lambda3 = 1)
parms = c('beta0','gamma1','beta1','beta2','beta3','lambda1','lambda2','lambda3','tau1','tau2','tau3')
...
save(thin, MCMC, df, mcmc, 
     publisher_labels, country_labels,
     file = 'data/8_results_poisson_errors_horseshoe.RData')
...
# to do, add author number predictions
# look at lambda chains